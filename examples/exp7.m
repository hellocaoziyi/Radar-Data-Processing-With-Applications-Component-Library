...
for iIter = 2:Target.nIter
    ...
    PCRBtemp = inv(J(:,:,iIter));
    PCRBci(iIter) = (PCRBtemp(1,1) + PCRBtemp(3,3)).^0.5;
end
...
PCRBtemp = inv(J(:,:,1));
PCRBciE(1) = (PCRBtemp(1,1) + PCRBtemp(3,3)).^0.5;
...
for iIter = 2:Target.nIter
    ...
    PCRBtemp = inv(J(:,:,iIter));
    PCRBciE(iIter) = (PCRBtemp(1,1) + PCRBtemp(3,3)).^0.5;
end
...
plot(Station.Xhat_monte(:,1),'Color','#D95319','LineStyle',':','LineWidth',1,'DisplayName','雷达1 KF RMSE');
plot(Station.Xhat_monte(:,2),'Color','#EDB120','LineStyle','--','LineWidth',1,'DisplayName','雷达2 KF RMSE');
plot(Station.Xhat_monte(:,3),'Color','#7E2F8E','LineStyle','-.','LineWidth',1,'DisplayName','雷达3 KF RMSE');
plot(Station.Xhat_ci_monte(1,:),'Color','#77AC30','LineStyle','-','LineWidth',1,'DisplayName','KF 滤波 CI RMSE');
grid on;
box on;
...
plot(Station.XEhat_monte(:,1),'Color','#D95319','LineStyle',':','LineWidth',1,'DisplayName','雷达1 EKF RMSE');
plot(Station.XEhat_monte(:,2),'Color','#EDB120','LineStyle','--','LineWidth',1,'DisplayName','雷达2 EKF RMSE');
plot(Station.XEhat_monte(:,3),'Color','#7E2F8E','LineStyle','-.','LineWidth',1,'DisplayName','雷达3 EKF RMSE');
plot(Station.XEhat_ci_monte(1,:),'Color','#77AC30','LineStyle','-','LineWidth',1,'DisplayName','EKF 滤波 CI RMSE');
grid on;
box on;
xlabel('仿真时间/s','FontSize',20);
ylabel('RMSE/m','FontSize',20);
